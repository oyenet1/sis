<div class="relative w-32 p-0 cursor-pointer" @click.away="sort = false" x-data="{ sort: false }" @click="sort= true">
  <p class="h-full w-full  px-3 py-3 text-xs font-medium bg-white border-none rounded-lg shadow text-gray-500">Sort By
  </p>
  {{-- <svg xmlns="http://www.w3.org/2000/svg" class="absolute w-6 h-6 font-bold top-2 right-2 icons-svg text-primary"
  ...
  <div @mouseleave="sort = false" x-show="sort"
    class="absolute text-gray-500 top-0 mt-12 z-50 divide-y divide-dashed bg-opacity-50 backdrop-blur-sm bg-blend-saturation divide-gray-200 space-y-2 w-48 p-4 rounded-lg bg-white shadow">
    @for ($i = 0; $i < 4; $i++)
      <p class="text-sm pt-2 tt hover:translate-x-2 cursor-pointer">Name</p>
    @endfor
  </div>
</div>
