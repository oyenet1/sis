...
<body class="font-fira">
  <div class="relative mx-auto max-w-7xl">
    ...
    <table class="w-full">
      <tr class=" w-full py-4 m-0  ">
        <th class="text-2xl font-medium">Class Report</th>
        <td class="text-right text-sm ml-auto" style="float: right;">
    ...
    <table class="w-full">
      ...
      <tbody>
        @foreach ($data as $class)
          <tr class="od text-sm border uppercase border-b">
            <td>{{ $loop->iteration }}</td>
            <td>{{ $class['name'] . ' ' . $class['section'] }}</td>
            <td class="keep-all">{{ $class['high'] ?? $class['type'] }}</td>
            <td>{{ $class['user']['first_name'] . ' ' . $class['user']['last_name'] }}</td>
            <td>
              <span>{{ count($class['students']) }}</span>
            </td>
          </tr>
        @endforeach
    ...
    <footer class="p-4 footer text-center justify-center text-sm">
      ...
      <div class="flex space-x-2 gap-2 items-center justify-center text-sm m-0 p-0">
        <span class="flex items-center justify-center text-sm p-0 px-2 m-0">
