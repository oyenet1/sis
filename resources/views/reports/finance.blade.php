...
<head>
  ...
  <style>
    ...
    .first {
      ...
      padding: 5px 10px;
      color: white;
      background-color: rgb(22, 163, 50);
    }
...
<body class="font-fira">
  <div class="relative mx-auto max-w-7xl">
    <table class="w-full" style="background-color: #E4EEFA">
      <tr class="w-full py-4 m-0 ">
        ...
        <td class="text-right bg-white " style="float: right; text-justify: center">
          <p class="m-0 text-lg font-medium text-center uppercase text-primary">AL-ANSER CENTER FOR <br> COMPREHENSIVE
            EDUCATION</p>
          <p class="m-0 italic text-center capitalize text-secondary">inspiring excellence</p>
        </td>
      </tr>
    </table>
    <table class="w-full">
      <tr class="w-full py-4 m-0 ">
        <th class="text-2xl font-medium">Finance Report</th>
        <td class="ml-auto text-sm text-right" style="float: right;">
    ...
    <table class="w-full">
      ...
      <tbody>
        @foreach ($data as $finance)
          <tr class="text-sm uppercase border border-b od">
            <td>{{ formatDate($finance['date']) }}</td>
            <td class="keep-all">{{ Str::limit($finance['description'], 50, '...') }}</td>
            <td>{{ $finance['type'] }}</td>
            <td>
              <span>{{ moneyFormat($finance['amount']) }}</span>
            </td>
            <td>
              <span class="{{ $loop->first ? 'first' : '' }}">{{ moneyFormat($finance['balance']) }}</span>
            </td>
          </tr>
        @endforeach
    ...
    <footer class="justify-center p-4 text-sm text-center footer">
      <p class="flex items-center justify-center p-0 m-0 text-sm">
        <img src="{{ asset('icons/location.svg') }}" alt="icons" class="text-white icon">
        {{-- <span class="text-white">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
            class="text-white bi bi-geo" viewBox="0 0 16 16">
      ...
      <div class="flex items-center justify-center gap-2 p-0 m-0 space-x-2 text-sm">
        <span class="flex items-center justify-center p-0 px-2 m-0 text-sm">
