<div class="flex flex-col w-full gap-6 py-8 lg:py-12">
  ...
  <x-modal class="max-w-lg">
    ...
    <div class="w-full gap-4 px-4 mx-auto mt-4 overflow-y-auto lg:px-12">
      ...
      <form wire:submit.prevent="continue" class="flex flex-col w-full gap-6 mx-auto my-4 overflow-y-auto max-h-max min-h-fit" x-data="{ login: 'email', id: true }">
        <span class="px-4 text-sm text-center">You need to add parent details before you can add student.
          Parent
          can be
          added
        ...
        <div class="flex items-center justify-center m-0 space-x-4 text-sm font-medium">
          <span class="px-4 py-2 capitalize rounded-lg cursor-pointer" :class="login === 'email' ? 'bg-background text-primary' : 'text-primary'" @click="login = 'email'">Parent
            email</span>
          <span class="px-4 py-2 capitalize rounded-lg cursor-pointer" :class="login === 'id' ? 'bg-background text-primary' : 'text-primary'" @click="login = 'id'">Parent
            id</span>
        ...
        <div class="w-full">
          ...
          <x-single-datalist wire:model.defer="parent" name="parent" :datas="$parents" />
          {{-- <input type="text" wire:model="parent" class="w-full bg-gray-100 border-0 rounded-lg focus:ring-0"
							list="opt">

						<datalist id="opt" class="p-2 bg-yellow-100">
							@foreach (userByRole(10) as $option)
								<option value="{{ $option->email }}">
          @endforeach
          </datalist> --}}

          <p class="p-1 text-sm 2xl:text-base">If parent is not registered yet. <a href="{{ route('parents') }}" class="tt text-primary hover:text-secondary">Add
              Parent</a>
          </p>
        </div>
        <button wire:loading.attr="disabled" class="tt submit-primary mx-auto block w-[331px] py-2 font-normal capitalize hover:bg-primary" type="submit">Continue
    ...
    <div class="w-full gap-4 px-4 mx-auto mt-4 overflow-y-auto lg:px-12">
      ...
      <!-- Modal body -->
      <form wire:submit.prevent="save" class="flex flex-col w-full gap-6 mx-auto my-4 overflow-y-auto max-h-max min-h-fit" x-data="{ login: 'id', id: true }">
        <span class="text-sm text-center">Make sure that the parent information is correct</span>

        <div class="w-full space-y-1">
          <h1 class="text-lg font-medium text-center lg:text-xl 2xl:text-2xl">Parent Information</h1>
          <p class="w-full p-3 pl-4 font-medium placeholder-gray-500 uppercase bg-gray-100 border-0 rounded-lg cursor-not-allowed peer tt text-dark focus:border-2 focus:border-primary focus:bg-white focus:outline-none" @disabled(true)>
            {{ $parentD->first_name . ' ' . $parentD->last_name }}
          </p>
          <p class="p-1 text-sm 2xl:text-base">If parent's/guardian's information is not correct, kindly
            click
        ...
        <button wire:loading.attr="disabled" class="tt submit-primary mx-auto w-[331px] py-2 font-normal capitalize hover:bg-primary" type="submit">save
  ...
  <section class="flex flex-col w-full gap-6 bg-background" x-data="{ active: 'students' }">
    ...
    <div class="flex flex-wrap items-center justify-between w-full">
      ...
      <div class="flex flex-col text-gray-500">
        <p class="">Total students:
          <strong class="text-black">{{ \App\Models\Student::all()->count() }}</strong>
        </p>
      </div>
    </div>
    {{-- buttons --}}
    <div class="flex items-center justify-between">
      <div class="flex items-center space-x-6">
        ...
        @elseif(in_array(currentUserFirstRole(), ['superadministrator', 'admin']))
        ...
        <tr class="even:bg-primary-light">
          ...
          <td class="">
            <span class="flex items-center space-x-3">
              <input type="checkbox" wire:model="checked" id="" value="{{ $student->id }}" class="block rounded whitespace-nowrap text-primary focus:outline-none focus:ring-primary">
          ...
          <td class="p-2 uppercase">
            @if ($student->clas)
            <span>{{ $student->clas->name . $student->clas->section }}</span>
            @endif
          </td>
          ...
          <td class="p-2 whitespace-nowrap">
            ...
            <span class="block w-20 py-2 text-sm font-normal text-center capitalize bg-opacity-25 rounded-lg shadow-sm bg-secondary text-secondary">{{ $student->status }}</span>
            @elseif($student->status == 'graduated')
            <span class="block w-20 py-2 text-sm font-normal text-center text-green-600 capitalize bg-green-600 bg-opacity-25 rounded-lg shadow-sm">{{ $student->status }}</span>
            @else
            <span class="block w-20 py-2 text-sm font-normal text-center capitalize bg-opacity-25 rounded-lg shadow-sm bg-primary text-primary">{{ $student->status }}</span>
            @endif
          </td>
          <td class="p-2 whitespace-nowrap">
            <div class="flex space-x-2 item-center">
              <a href="{{ route('students.profile', [$student->guardian->first_name, $student->id]) }}" class="w-8 h-8 p-2 text-blue-600 border border-blue-600 rounded-full cursor-pointer tt hover:-translate-y-1">
              ...
              <span wire:click="confirmDelete({{ $student->id }})" class="w-8 h-8 p-2 text-red-600 border border-red-600 rounded-full cursor-pointer tt hover:-translate-y-1">
  ...
  @else
  <x-empty-table comment="There is no student register yet, you can add student by first creating the guardian/parent account and then use the parent email/id to register student into the system " title="student">
