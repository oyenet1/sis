...
class Students extends Component
{
    ...
    function save()
    {
        ...
        try {
            ...
            if ($student) {
                ...
                // refactor the student id according to their class
                $student = $student->update(['admission_id' => changeId($student->student_id, $student->clas->school->short)]);
    ...
    {
        $parents = userByRole(10);
        $titles = ['mr', 'mrs', 'dr', 'prof.', 'eng.', 'madam', 'sir', 'lady', 'lord'];
        $term = "%$this->search%";
        $students = Student::with(['clas.school', 'guardian'])->where('first_name', 'LIKE', $term)
            ->orWhere('last_name', 'LIKE', $term)
            ->orWhere('student_id', 'LIKE', $term)
            ->orWhere('admission_id', 'LIKE', $term)
            ->orWhere('email', 'LIKE', $term)
            ->orWhereHas('clas', function (Builder $query) {
                $term = "%$this->search%";
        ...
        return view('livewire.students', compact(['students', 'parents']))->layout('layouts.dashboard');
    }
}